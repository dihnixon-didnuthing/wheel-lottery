...
$("#add_prize_row").live("click", function() {
  ...
  var pb = document.getElementById("prizes_body");
  ...
  tr.innerHTML =
    "<td></td>" +
    "<td contenteditable='true' data-ph='Tên giải...' style='min-width:100px'></td>" +
    "<td contenteditable='true' data-ph='SL' style='min-width:30px'></td>";
  ...
  function tryCommit() {
    ...
    document.getElementById("badge_p").textContent = PRIZES.length;
    refreshPanel();
  }
    ...
    cell.addEventListener("keydown", function(e) {
      if (cell === qtyCell) {
        ...
        if (allowed.indexOf(e.key) === -1 && !/^\d$/.test(e.key)) {
          e.preventDefault();
          showNotif("Số lượng chỉ nhận số nguyên dương", "error");
          return;
        }
      }

...
function downloadTemplatesEmployee() {
  // Employee template
  var empWb = XLSX.utils.book_new();
  var empWs = XLSX.utils.aoa_to_sheet([["ID", "Name", "Department"]]);
  XLSX.utils.book_append_sheet(empWb, empWs, "Sheet1");
  XLSX.writeFile(empWb, "template_ds_nhan_vien.xlsx");
  showNotification("Đã tải file mẫu danh sách nhân viên.", "success");
}

function downloadTemplatesPrize() {
  // Prize template
  var prizeWb = XLSX.utils.book_new();
  var prizeWs = XLSX.utils.aoa_to_sheet([["Prize Name", "Quantity"]]);
  XLSX.utils.book_append_sheet(prizeWb, prizeWs, "Sheet1");
  XLSX.writeFile(prizeWb, "template_ds_giai_thuong.xlsx");

  showNotification("Đã tải file mẫu danh sách giải thưởng.", "success");
}
